...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    // your code here:
    float cos_value = max(u_UseBlinn ? dot(normal, normalize(lightDir + viewDir)) : dot(viewDir, reflect(-lightDir, normal)), 0.0);
    return diffuseColor * lightIntensity * max(dot(lightDir, normal), 0.0) + specularColor * lightIntensity * pow(cos_value, shininess);
}
...
vec3 GetNormal() {
    ...
    return normalize(vn * (1. - u_BumpMappingBlend) + bumpNormal * u_BumpMappingBlend);
}
